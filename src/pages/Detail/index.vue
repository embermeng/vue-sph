...
export default {
        ...
        async addCart() {
            ...
            try {
                ...
                this.spuSaleAttrList.forEach((saleAttr) => {
                    let i = saleAttr.spuSaleAttrValueList.find((cur) => {
                        return cur.isChecked == "1"
                    })
                    isCheckedAttr.push(i)
                })
                this.skuInfo.skuSaleAttrValueList = isCheckedAttr
                // 路由跳转，需要将产品信息带给下一级路由组件
                // 简单的参数通过query形式给路由组件传递过去
                this.$router.push({
                    name: "addcartsuccess",
                    query: { count: this.proCount },
                });
                // 复杂的参数（对象）通过会话存储（sessionStorage）传递
    ...
    mounted() {
        // 获取产品详情信息
        this.$store.dispatch("getProDetail", this.$route.params.proId);
    },
};
</script>
